...
IGNITION_RATE = 1 # Increase the time required for the fire to spread #2
FIRST_JUMP_RATIO = 1.2 #3

AGENT_AREA = 3
ADD_CENTER_MOISTURE = 0.4 #3
ADD_SURROUNDING_MOISTURE = 0.2 #4
...
class FireMap:
    ...
    """
    def __init__(self, board: np.array, wind: tuple[float, float]=None, humidity: float = 0) -> None:
    ...
    def show(self, iter) -> None:
        ...
        plt.pause(0.1)  # Needed for non-Jupyter environments
        plt.close()        

    def render(self, iter: int) -> str:
        """
        Render the fire map as a base64 image string for web display.
        This matches the output of `.show()` but sends it to the web instead of a GUI.
        """
        # Set up figure
        fig, ax = plt.subplots(figsize=(4, 4))  # Match size to original
        ...
        # Plot fire intensity
        ax.imshow(self.state[:, :, INTENSITY], cmap=intensity_cm, interpolation='none', vmin=-1, vmax=1)
        # Overlay moisture
        ax.imshow(self.state[:, :, MOISTURE], cmap=moisture_cm, interpolation='none', alpha=MOISTURE_VISIBILITY, vmin=0, vmax=1)

        # Title & axis
        ax.set_title(f"Wind=({self.state[0, 0, WIND_X]:.1f}, {self.state[0, 0, WIND_Y]:.1f}) | Time={self.time}")
        ax.invert_yaxis()
        ax.axis('off')

        # Draw wind vector
        wind_x, wind_y = self.state[0, 0, WIND_X], self.state[0, 0, WIND_Y]
        rounded_x, rounded_y = FireMap._round_to_8_directions(wind_x, wind_y)
        ax.quiver(MAP_SIZE//2, MAP_SIZE//2, rounded_x, rounded_y, scale=3, color='black', width=0.02)

        # Draw previous actions (firefighter locations)
        for (x, y) in self.prev_actions:
            ax.scatter(AGENT_AREA * x + 1, AGENT_AREA * y + 1, c='white')
...
def make_board(size: int = MAP_SIZE, start_intensity: int = START_INTENSITY, num_points: int = STARTING_FIRES):
    ...
    # TODO: refactor
    coords = np.random.randint(30, 60, size=(num_points, 2))
    for x, y in coords:
